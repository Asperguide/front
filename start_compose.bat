...
@REM .........#####...#####.........
@REM /STOP
@REM PROJECT: AsperHeader
@REM FILE: start_compose.bat
@REM CREATION DATE: 11-10-2025
@REM LAST Modified: 21:59:49 16-10-2025
@REM DESCRIPTION: 
...
SET "ENV_PATH=back-end\docker"
IF NOT EXIST "%ENV_PATH%" (
  ECHO Error: %ENV_PATH% folder not found. Please ensure you are running this script from the correct directory.
  exit 1
)
...
SET "ENV_PATH=.front.env"
IF NOT EXIST "%ENV_PATH%" (
  type NUL > "%ENV_PATH%"
  ECHO Notice: %ENV_PATH% not found — Creating an empty file.
)

SET "ENV_PATH=back-end\.env"
IF NOT EXIST "%ENV_PATH%" (
  type NUL > "%ENV_PATH%"
  ECHO Notice: %ENV_PATH% not found — Creating an empty file.
)

SET "ENV_PATH=back-end\docker\.db.env"
IF NOT EXIST "%ENV_PATH%" (
  type NUL > "%ENV_PATH%"
  ECHO Notice: %ENV_PATH% not found — Creating an empty file.
)

SET "ENV_PATH=back-end\docker\.redis.env"
IF NOT EXIST "%ENV_PATH%" (
  type NUL > "%ENV_PATH%"
  ECHO Notice: %ENV_PATH% not found — Creating an empty file.
)

SET "DATABASE_CACHE_DIR=back-end\docker\db\data"
IF NOT EXIST "%DATABASE_CACHE_DIR%" (
  mkdir "%DATABASE_CACHE_DIR%"
  echo "Notice: %DATABASE_CACHE_DIR% not found, creating the directory."
)

SET "REDIS_CACHE_DIR=back-end\docker\redis\data"
IF NOT EXIST "%REDIS_CACHE_DIR%" (
  mkdir "%REDIS_CACHE_DIR%"
  echo "Notice: %REDIS_CACHE_DIR% not found, creating the directory."
)

:: Compose file path (relative to repo root) — use root compose so users can run `docker compose up --build` from repo root
